##
#
# Unitree G1 (29 dof) reference trajectory tracking with MPC.
#
##
...
class G1TrackingMPC(ModelPredictiveControl):
    ...
    def __init__(self, dynamics_config, mpc_config, track_config, X_ref, dt_ref):
        ...
        print(f"[G1TrackingMPC] motion={c.name}  tracking: state={self.track_state}  "
...
if __name__ == "__main__":
    ...
    # pick the motion config by name:  python g1_tracking_mpc.py <motion>
    parser = argparse.ArgumentParser(
        description="Unitree G1 (29 dof) reference-tracking MPC; pick a motion from config.CONFIGS.")
    parser.add_argument("motion", choices=sorted(CONFIGS),
                        help="which motion config to track (see examples/g1_tracking_mpc/config.py)")
    args = parser.parse_args()
    cfg = CONFIGS[args.motion]
    print(f"[g1_tracking_mpc] motion config: {cfg.name}")
    ...
    X_ref, dt_ref = load_reference(cfg.traj_path(ROOT), _m, centered=True)
    ...
    mpc = G1TrackingMPC(dyn_cfg, mpc_cfg, cfg, X_ref, dt_ref)
    x_init = X_ref[0].copy()
    ...
    # replay via  replay.py g1_tracking_<name>   (the stem is unique, so no folder needed)
    stem = f"g1_tracking_{cfg.name}"
    save_path = os.path.join(ROOT, "examples", "g1_tracking_mpc", stem + ".npz")
